function utils = robot_utils()
    ...
    utils = struct(...
        ...
        'calculateMakespan', @calculateMakespan, ...
        'calculateOptimalMakespan', @calculateOptimalMakespan, ...
        'updateRobotWorkload', @updateRobotWorkload ...
    );
    
    % Define nested functions
    function robots = createRobots(num_robots, env)
    ...
    function makespan = calculateMakespan(assignment, tasks, robots)
        ...
        execution_order = determineExecutionOrder(tasks);
    ...
    function execution_order = determineExecutionOrder(tasks)
    ...
    function path_length = calculateCriticalPathLength(task_id, tasks, memo)
        ...
        if ~isfield(tasks, 'prerequisites') || isempty(tasks(task_id).prerequisites)
            ...
            for i = 1:numel(tasks(task_id).prerequisites)
                ...
                prereq_length = calculateCriticalPathLength(prereq_id, tasks, memo);
    ...
    function makespan = calculateOptimalMakespan(tasks, robots)
        ...
        for i = 1:length(tasks)
            path_length = calculateCriticalPathLength(i, tasks, memo);
    ...
    function robots = updateRobotWorkload(robots, old_assignment, new_assignment, task_index, task_time)
