...
function [metrics, converged] = local_runAuctionSimulation(params, env, robots, tasks, visualize)
    ...
    % Initialize auction data directly
    ...
    try
        % Use utils_manager to get task_utils
        utils = utils_manager();
        available_tasks = utils.task.findAvailableTasks(tasks, []);
    catch
    ...
    for iter = 1:max_iterations
        ...
        if isfield(params, 'failure_time') && iter == params.failure_time - 1
        ...
        if isfield(params, 'failure_time') && iter == params.failure_time
        ...
        if visualize
            try
                % Use utils_manager to get environment_utils
                utils = utils_manager();
                subplot(2, 3, [1, 4]);
                utils.env.visualizeEnvironment(env, robots, tasks, auction_data);
        ...
        try
            ...
            available_tasks = utils.task.findAvailableTasks(tasks, completed_tasks);
        catch
        ...
        if isfield(params, 'failed_robot') && ~isempty(params.failed_robot) && params.failure_time < inf && metrics.recovery_time == 0
    ...
    % Calculate makespan
    try
        % Use utils_manager to get robot_utils
        utils = utils_manager();
        metrics.makespan = utils.robot.calculateMakespan(auction_data.assignment, tasks, robots);
        metrics.optimal_makespan = utils.robot.calculateOptimalMakespan(tasks, robots);
    catch
    ...
    if isfield(params, 'failed_robot') && ~isempty(params.failed_robot) && params.failure_time < inf
