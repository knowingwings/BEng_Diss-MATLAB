function utils = auction_utils()
    ...
    function [auction_data, new_assignments, messages] = distributedAuctionStep(auction_data, robots, tasks, available_tasks, params, varargin)
        ...
        for i = 1:num_robots
            ...
            for j = available_tasks
                ...
                single_bid = calculateImprovedBid(i, j, robot_workloads(i), workload_ratio, workload_imbalance, auction_data, params, iter);
    ...
    function bid = calculateBid(robot_id, task_id, robot_workload, auction_data, params)
        % This is the original function but isn't used anymore
        bid = 0;
    end
    
    function bid = calculateImprovedBid(robot_id, task_id, robot_workload, workload_ratio, workload_imbalance, auction_data, params, varargin)
        % Handle optional iter parameter for backward compatibility
        if length(varargin) >= 1
            iter = varargin{1};
        else
    ...
    function auction_data = initiateRecovery(auction_data, robots, tasks, failed_robot_id)
    ...
    function x_consensus = consensusUpdate(x_i, x_others, last_update_times, gamma, lambda)
    ...
    function [metrics, converged] = runAuctionSimulation(params, env, robots, tasks, visualize)
        ...
        auction_data = initializeAuctionData(tasks, robots);
        ...
        for iter = 1:max_iterations
            ...
            if iter == params.failure_time && ~isempty(params.failed_robot)
                ...
                auction_data = initiateRecovery(auction_data, robots, tasks, params.failed_robot);
            ...
            [auction_data, new_assignments, messages] = distributedAuctionStep(auction_data, robots, tasks, available_tasks, params, iter);
            metrics.messages = metrics.messages + messages;
            
            % Diagnostic output every 10 iterations
            if visualize && mod(iter, 10) == 0
                analyzeTaskAllocation(auction_data, tasks);
            end
            ...
            if visualize
                try
                    subplot(2, 3, [1, 4]);
                    env_fns.visualizeEnvironment(env, robots, tasks, auction_data);
            ...
            try
                ...
                available_tasks = task_fns.findAvailableTasks(tasks, completed_tasks);
            catch
        ...
        % Calculate makespan
        try
            metrics.makespan = robot_fns.calculateMakespan(auction_data.assignment, tasks, robots);
            metrics.optimal_makespan = robot_fns.calculateOptimalMakespan(tasks, robots);
        catch
