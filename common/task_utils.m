function utils = task_utils()
    ...
    utils = struct(...
        ...
        'calculateTaskCriticality', @calculateTaskCriticality, ...
        'calculateDependencyDepth', @calculateDependencyDepth, ...
        'isOnCriticalPath', @isOnCriticalPath ...
    );
    
    % Define nested functions
    function tasks = createTasks(num_tasks, env)
        ...
        grid_points = getGridPoints(grid_size, env);
    ...
    function grid_points = getGridPoints(grid_size, env)
    ...
    function tasks = addTaskDependencies(tasks, probability)
    ...
    function available_tasks = findAvailableTasks(tasks, completed_tasks)
    ...
    function criticality = calculateTaskCriticality(task_id, tasks)
    ...
    function dependency_depth = calculateDependencyDepth(task_id, tasks)
        ...
        if isempty(tasks(task_id).prerequisites)
            ...
            for i = 1:length(tasks(task_id).prerequisites)
                ...
                depths(i) = calculateDependencyDepth(prereq_id, tasks);
            end
    ...
    function is_critical = isOnCriticalPath(task_id, tasks)
        ...
        for i = 1:length(tasks)
            all_depths(i) = calculateDependencyDepth(i, tasks);
            all_criticalities(i) = calculateTaskCriticality(i, tasks);
        end
